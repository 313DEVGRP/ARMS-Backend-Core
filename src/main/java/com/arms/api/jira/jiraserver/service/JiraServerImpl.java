...
public class JiraServerImpl extends TreeServiceImpl implements JiraServer{
    ...
    public List<JiraIssueStatusEntity> 서버_이슈상태_가져오기(JiraServerEntity jiraServerEntity) throws Exception {
        ...
        List<JiraIssueStatusEntity> 지라이슈상태_목록 = jiraIssueStatus.getChildNode(이슈상태_검색전용);
...
//        }
        ...
        for (T 엔진_엔티티 : 가져온_엔티티_목록) {
            ...
            try {
                if (백엔드_엔티티 instanceof JiraProjectEntity || EntityType.프로젝트 == 갱신항목) {
                    ...
                    결과 = (E) 이슈상태_저장_또는_업데이트((지라이슈상태_데이터) 엔진_엔티티, (JiraIssueStatusEntity) 백엔드_엔티티, 서버유형);
                }
    ...
    private JiraIssueStatusEntity 이슈상태_저장_또는_업데이트(지라이슈상태_데이터 엔진_이슈상태, JiraIssueStatusEntity 백엔드_이슈상태, String 서버유형) throws Exception {
        ...
        // 클라우드 지라 서버의 경우 프로젝트 이슈유형별 상태를 프리셋하는 로직 추가
        // 현재 프리셋 방법을 하드코딩 적용 But  테이블 정의, yml 정의 방법 중 진행 필요
        // 1 대 N의 경우 상태가 랜덤으로 업데이트 되는 현상 발생할 예정이므로 변경 필요
        if (StringUtils.equals(ServerType.JIRA_CLOUD.getType(), 서버유형)) {
            if (엔진_이슈상태.getStatusCategory() != null && StringUtils.isNotEmpty(엔진_이슈상태.getStatusCategory().getKey())) {
                if (StringUtils.equals(엔진_이슈상태.getStatusCategory().getKey(), "new")) {
                    백엔드_이슈상태.setC_req_state_mapping_link(10L);
                }
                else if (StringUtils.equals(엔진_이슈상태.getStatusCategory().getKey(), "indeterminate")) {
                    백엔드_이슈상태.setC_req_state_mapping_link(11L);
                }
                else if (StringUtils.equals(엔진_이슈상태.getStatusCategory().getKey(), "done")) {
                    백엔드_이슈상태.setC_req_state_mapping_link(12L);
                }
            }
        }
    ...
    @Override
    @Transactional
    public boolean ALM_서버_전체_항목_갱신(JiraServerEntity jiraServerEntity) {
